...
jd_csv_path = os.path.join(current_dir, 'data', 'jd_structured_data.csv')
jd_df = pd.read_csv(jd_csv_path)

# Load the extracted resume skills:
resume_dir = os.path.join(current_dir, 'utilities', 'resumes')
resume_path = os.path.join(resume_dir, 'CV.pdf')
skills = []
skills.append(' '.join(word for word in skills_extraction.skills_extractor(resume_path)))
